...
const HomePage = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (selectedOption && subject && branch && semester) {
            try {
                const response = await fetch(`http://s3-storage-env.eba-2cqjhzga.eu-north-1.elasticbeanstalk.com/paper/getPaper?branch=${branch}&semester=${semester}&subject=${subject}`);
                if (!response.ok) {  
